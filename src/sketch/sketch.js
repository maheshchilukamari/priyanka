...
export default (parent, text) => (sketch) => {
  ...
  const fillDots = (width, height) => {
    ...
    if (width > height) {
      ...
      firstWordPoints = font.textToPoints(`${firstWord} ${secondWord}`, width * 0.05, height * 0.33, width * 0.12)
      secondWordPoints = font.textToPoints(thirdWord, width * 0.13, height * 0.65, width * 0.15);
    //  secondWordPoints = font.textToPoints(thirdWord, width * 0.05, height * 0.83, width * 0.3);
     // thirdWordPointsr = font.textToPoints(thirdWord, width * 0.05, height * 0.83, width * 0.3);
    } else {
